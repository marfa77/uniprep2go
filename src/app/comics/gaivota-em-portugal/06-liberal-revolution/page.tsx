import type { Metadata } from "next";
import Image from "next/image";
import Link from "next/link";
import { GaivotaEpisodeSeoSections } from "@/components/comics/gaivota-episode-seo-sections";
import { SiteFooter } from "@/components/site-footer";
import { SiteHeader } from "@/components/site-header";
import { gaivotaEpisodePageTitle, gaivotaEpisodes, gaivotaSeries } from "@/lib/gaivota-comics";
import { withAiMetadata } from "@/lib/llm-meta";
import { siteConfig } from "@/lib/site";
import { btnPrimary, btnSecondary } from "@/lib/ui-button-classes";

const pagePath = "/comics/gaivota-em-portugal/06-liberal-revolution";
const ep = gaivotaEpisodes.find((e) => e.id === "06-liberal-revolution")!;
const checkout = ep.gumroadCheckoutUrl ?? gaivotaSeries.gumroadCheckoutUrl;

const directAnswer =
  "Gaivota em Portugal Episode 6 — Liberal Revolution (Porto, 1820): a paid noir Portuguese history comic PDF ($5 on Gumroad). Includes a factual history brief, a complete 5-page story, and a 100-word PT→EN glossary. Episode 1 (1755) is free on this site.";

export const metadata: Metadata = withAiMetadata(
  {
  title: gaivotaEpisodePageTitle("06-liberal-revolution"),
  description: directAnswer,
  alternates: { canonical: `${siteConfig.url}${pagePath}` },
  openGraph: {
    title: "Liberal Revolution — Gaivota comic ($5)",
    description: directAnswer,
    url: `${siteConfig.url}${pagePath}`,
    images: [
      {
        url: `${siteConfig.url}/comics/gaivota-em-portugal/06-liberal-revolution/ep06-cover.webp`,
      },
    ],
  },
  },
  {
    aiDescription: directAnswer,
    aiCategory: "comics;portuguese-history;gaivota",
    path: pagePath,
  },
);

export default function Ep06LiberalRevolutionPage() {
  return (
    <div className="min-h-screen bg-[#ebe4d8] text-[#0e0e0e]">
      <SiteHeader />
      <main className="mx-auto w-full max-w-3xl px-4 pb-16 pt-8 sm:px-8">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8b1f3d]">
          Ep.06 · $5 · Gumroad
        </p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">
          Revolução Liberal
        </h1>
        <p className="mt-2 text-base text-[#5c574e] sm:text-lg">Porto · 1820</p>

        <figure className="mt-8">
          <Image
            alt="Cover: Liberal Revolution — Gaivota comic"
            className="mx-auto h-auto w-full max-w-[20rem] sm:max-w-[24rem]"
            height={900}
            priority
            sizes="(max-width: 640px) 80vw, 24rem"
            src="/comics/gaivota-em-portugal/06-liberal-revolution/ep06-cover.webp"
            width={675}
          />
        </figure>

        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <a className={btnPrimary} href={checkout} rel="noreferrer">
            Buy Ep.06 PDF — $5
          </a>
          <Link className={btnSecondary} href="/comics/gaivota-em-portugal/01-1755-earthquake">
            Read free Ep.01 first
          </Link>
          <Link className={btnSecondary} href={gaivotaSeries.hubPath}>
            Series hub
          </Link>
        </div>

        <p className="mt-8 max-w-2xl text-base leading-relaxed text-[#3a342c] sm:text-lg">
          {directAnswer}
        </p>
        <ul className="mt-5 list-disc space-y-1 pl-5 text-[#3a342c]">
          <li>
            Printable <strong>PDF</strong> download
          </li>
          <li>History brief + noir 5-page comic with ending</li>
          <li>100 new Portuguese words · English glossary</li>
          <li>Human-scale story — press and oaths, not gore</li>
        </ul>
        <GaivotaEpisodeSeoSections episodeId="06-liberal-revolution" />
      </main>
      <SiteFooter />
    </div>
  );
}
