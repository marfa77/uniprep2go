...
const pagePath = "/comics/gaivota-em-portugal/07-republic";
const ep = gaivotaEpisodes.find((e) => e.id === "07-republic")!;
const checkout = ep.gumroadCheckoutUrl ?? gaivotaSeries.gumroadCheckoutUrl;

const directAnswer =
  "Gaivota em Portugal Episode 7 — Republic (5 October 1910): a paid noir Portuguese history comic PDF ($5 on Gumroad). Includes a factual history brief, a complete 5-page story, and a 100-word PT→EN glossary. Episode 1 (1755) is free on this site.";

export const metadata: Metadata = {
  title: "Republic (1910) — Portuguese History Comic | Gaivota em Portugal",
  description: directAnswer,
  alternates: { canonical: `${siteConfig.url}${pagePath}` },
  openGraph: {
    title: "Republic 1910 — Gaivota comic ($5)",
    description: directAnswer,
    url: `${siteConfig.url}${pagePath}`,
    images: [
      {
        url: `${siteConfig.url}/comics/gaivota-em-portugal/07-republic/ep07-cover.webp`,
      },
    ],
  },
};

export default function Ep07RepublicPage() {
  return (
    <div className="min-h-screen bg-[#ebe4d8] text-[#0e0e0e]">
      ...
      <main className="mx-auto w-full max-w-3xl px-4 pb-16 pt-8 sm:px-8">
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">República</h1>
        <p className="mt-2 text-base text-[#5c574e] sm:text-lg">5 October 1910 · Lisboa</p>

        <figure className="mt-8">
          <Image
            alt="Cover: Republic 1910 — Gaivota comic"
            className="mx-auto h-auto w-full max-w-[20rem] sm:max-w-[24rem]"
            height={900}
            priority
            sizes="(max-width: 640px) 80vw, 24rem"
            src="/comics/gaivota-em-portugal/07-republic/ep07-cover.webp"
            width={675}
          />
        </figure>
        ...
        <ul className="mt-5 list-disc space-y-1 pl-5 text-[#3a342c]">
          ...
          <li>Human-scale story — flag and streets, not gore</li>
        </ul>
        <GaivotaEpisodeSeoSections episodeId="07-republic" />
      </main>
      <SiteFooter />
    </div>
  );
