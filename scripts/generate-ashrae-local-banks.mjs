...
function q(prompt, correct, wrong, explanation, difficulty = "medium", correctId = "a") {
  const ids = ["a", "b", "c", "d"];
  const wrongIds = ids.filter((id) => id !== correctId);
  const options = ids.map((id) => ({
    id,
    text: id === correctId ? correct : wrong[wrongIds.indexOf(id)],
  }));
  const distractorExplanations = Object.fromEntries(
    wrongIds.map((id, i) => [id, wrong[i].replace(/^Wrong: /, "") || `Incorrect — ${wrong[i].slice(0, 80)}`])
  );
  return { prompt, options, correctOptionId: correctId, explanation, distractorExplanations, difficulty };
}

// Helper to build distractor explanations from short wrong reasons
function makeQ(prompt, correct, wrongReasons, explanation, difficulty = "medium", correctId = "a") {
  const ids = ["a", "b", "c", "d"];
  const wrongIds = ids.filter((id) => id !== correctId);
  const options = ids.map((id) => ({
    id,
...
// Continue with beap, bcxp, chd-hbdp, opmp - I'll add them in the same file
// Due to length, I'll write the remaining topics in a second write operation
